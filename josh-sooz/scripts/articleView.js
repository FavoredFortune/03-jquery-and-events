...
articleView.handleCategoryFilter = function() {
  ...
  $('#category-filter').on('change', function() {
    if ($(this).val()){
      $('article').hide();
      $(`article[data-category="${$(this).val()}"]`).fadeIn(FadeIn_TimeOut);
    }
    else {
      // DONE: If the <select> menu was changed to an option that is blank, we should first show all the articles, except the one article we are using as a template.
      $('article').fadeIn(FadeIn_TimeOut);
...
articleView.handleMainNav = function() {
  ...
  //GOT THIS TO STOP BREAKING NAV, BUT DOESN"T WORK CORRECTLY

  $('.main-nav').on('click', '.tab', function(){
    if($(this).val()){
    //hide all tab content when clicked on
      $('.tab').hide();
      $(`#${$(this).data('tab')}`).fadeIn(FadeIn_TimeOut);
    } else{
      $('.tab').fadeIn(FadeIn_TimeOut);
    }
    $('.tab').val('');
  })
